...
const ReportGen: React.FC<ReportGenProps> = ({ items, onRemoveItem, history, onSaveToHistory, onFinishSession, externalLoadItem, onHistoryClick, onDraftSave, onDraftLoad, onReset }) => {
  ...
  const handleShareWithFile = async () => {
    if (!editorRef.current) return;
    setIsDownloading(true); // Re-use loading state for share preparation
    try {
      const opt = {
        margin: [15, 15, 15, 15],
      ...
      const worker = html2pdf().set(opt).from(editorRef.current);
      ...
      if (navigator.share && navigator.canShare && navigator.canShare({ files: [file] })) {
        ...
        // Simple share without file if not supported
        await navigator.share({
          title: reportTitle,
          text: `Check out this report: ${reportTitle}`
        });
      ...
      alert('Your browser/device might not support sharing PDF files directly. Consider using the "Direct Email" or "WhatsApp" link options below.');
    } finally {
      setIsDownloading(false);
    }
  };

  const handleShare = (platform: 'email' | 'whatsapp' | 'webshare') => {
      ...
      <div className={`flex justify-between items-center bg-white p-4 rounded-xl border border-slate-200 shadow-sm no-print fixed top-16 right-0 z-40 transition-all duration-300 ${sidebarExpanded ? 'left-64' : 'left-16'}`} style={{ height: 'auto', minHeight: '64px' }}>
        ...
        <div className="flex gap-2">
          ...
          <div className="relative">
            ...
            {showShareMenu && (
              <div className="absolute top-full right-0 mt-3 w-56 bg-white rounded-2xl shadow-2xl border border-slate-100 overflow-hidden z-50 animate-in fade-in zoom-in-95 p-2">
                <div className="text-[10px] font-bold text-slate-400 uppercase mb-2 ml-2 tracking-wider">Share Stack</div>
                <div className="space-y-1">
                  <button onClick={() => handleShare('webshare')} className="w-full text-left px-4 py-3 text-sm font-bold text-indigo-700 hover:bg-indigo-50 rounded-xl flex items-center justify-between group transition-all">
                    <div className="flex items-center gap-3">
                      <div className="p-2 bg-indigo-100 rounded-lg group-hover:bg-indigo-200 transition-colors"><Share2 size={16} /></div>
                      <div className="flex flex-col">
                        <span>Share with Attachment</span>
                        <span className="text-[9px] text-indigo-400 font-normal mt-0.5">Recommended for WhatsApp/Email files</span>
                      </div>
                    </div>
                    <ChevronRight size={14} className="text-indigo-300" />
                  </button>
                  <button onClick={() => handleShare('whatsapp')} className="w-full text-left px-4 py-3 text-sm font-bold text-green-700 hover:bg-green-50 rounded-xl flex items-center justify-between group transition-all">
                    <div className="flex items-center gap-3">
                      <div className="p-2 bg-green-100 rounded-lg group-hover:bg-green-200 transition-colors"><MessageCircle size={16} /></div>
                      <div className="flex flex-col">
                        <span>WhatsApp (Link Only)</span>
                        <span className="text-[9px] text-green-400 font-normal mt-0.5">Quick text reference</span>
                      </div>
                    </div>
                    <ChevronRight size={14} className="text-green-300" />
                  </button>
                  <button onClick={() => handleShare('email')} className="w-full text-left px-4 py-3 text-sm font-bold text-blue-700 hover:bg-blue-50 rounded-xl flex items-center justify-between group transition-all">
                    <div className="flex items-center gap-3">
                      <div className="p-2 bg-blue-100 rounded-lg group-hover:bg-blue-200 transition-colors"><Mail size={16} /></div>
                      <div className="flex flex-col">
                        <span>Email (Link Only)</span>
                        <span className="text-[9px] text-blue-400 font-normal mt-0.5">Direct mail app</span>
                      </div>
                    </div>
