...
import { generateColumnStatistics, imputeMissingValues, renameColumnInDataset, removeRowsWithMissing, dropColumn } from '../services/dataService';
...
const DataQuality: React.FC<DataQualityProps> = ({ report, data, onNext, onAddToReport, onDataUpdate }) => {
  ...
  const applyFix = (strategy: 'mean' | 'mode' | 'custom' | 'remove_rows' | 'drop_col') => {
          ...
          <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-50 flex items-center justify-center animate-in fade-in">
              <div className="bg-white rounded-xl shadow-2xl p-6 w-96 border border-slate-200 transform scale-100 animate-in zoom-in-95 relative overflow-hidden">
                  ...
                  <div className="space-y-3">
                      ...
                      >
                          <div className="font-semibold text-red-700 text-sm flex items-center gap-2"><Trash2 size={14}/> Remove Affected Rows</div>
                          <div className="text-xs text-red-500 mt-0.5">Delete {selectedFixCol.missingCount} rows. Use with caution.</div>
                      </button>
