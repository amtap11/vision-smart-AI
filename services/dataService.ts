...
export const detectOutliersZScore = (data: PatientRecord[], column: string, threshold: number = 3): number => {
  const values = data.map(d => Number(d[column])).filter(v => !isNaN(v) && v !== null);
  ...
  return values.filter(v => Math.abs((v - mean) / stdDev) > threshold).length;
};

export const removeOutliersZScore = (data: PatientRecord[], column: string, threshold: number = 3): PatientRecord[] => {
  const values = data.map(d => Number(d[column])).filter(v => !isNaN(v) && v !== null);
  ...
  return data.filter(d => {
    const v = Number(d[column]);
    if (typeof d[column] !== 'number' || d[column] === null) return true;
    return Math.abs((v - mean) / stdDev) <= threshold;
  });
};
 ...
 */
export const standardizeDates = (data: PatientRecord[], column: string): PatientRecord[] => {
  return data.map(row => {
    const val = row[column];
    if (!val) return row;
...
export const calculateTTest = (groupA: number[], groupB: number[]): { tStatistic: number; pValue: number; degreeOfFreedom: number } | null => {
  ...
  // Simple approximation of p-value for a two-tailed T-test
  // This is a rough estimation for demonstration; in a real app, use a more precise distribution function
  const pValue = 2 * (1 - Math.atan(Math.abs(tStatistic)) / (Math.PI / 2));

  return { tStatistic, pValue, degreeOfFreedom: df };
};

export const calculateChiSquare = (observed: number[][]): { chiSquare: number; pValue: number; df: number } | null => {
  ...
  const colSums = new Array(cols).fill(0).map((_, j) => observed.reduce((sum, row) => sum + row[j], 0));
  ...
  // Simple p-value approximation for Chi-Square
